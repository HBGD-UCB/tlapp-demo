...
source("C:/Users/andre/Dropbox/HBGDki figures/Example risk factor functions.R")
```
...
-Options for app:
   ...
   - Pick no subgroup, or a subgroup to stratify the analysis with
...
Output of app:
   ...
   - Saved plot objects and saved results dataframe
	...
	2. Set Y, A, and W (and subgroup D)
		a. Allow for vector of Y, A, W?
			i. List of W if different W per A or Y (avoid any on the causal pathway)
			ii. Right now, I'm median imputing W and making imputation flag indicators. Better way?
			
	3. Set reference level of A
	4. Drop any imputed A
	5. Get mean Y + 95% CI for each level of A
	6. For each cohort (study + country), and each level of A, estimate RR compared to reference
		a. If n_Y=1 is less than 5, don't estimate? 
	7. Estimate PAF shifting distribution of A to A_ref
	8. Create dataframe of results with the following columns:
		a. Study 
		b. Country
		c. Outcome Y (name, not actual data)
		d. Risk factor A
		e. Level of A ("a")
		f. N with A=a
		g. Mean Y_A=a
			i. CI for  Y_A=a (2 columns)
		h. If continuous Y: ATE of E(Y_A=a) - E(Y_A=ref)
			CI of ATE of E(Y_A=a) - E(Y_A=ref) (2 columns)
		i. If binary Y:RR and RD of P(Y_A=a)/P(Y_A=ref) (2 columns)
			i. CI of RR and RD of P(Y_A=a)/P(Y_A=ref) (4 columns)
		j. PAF (NA for rows where level of A is not the reference level)
			i. CI of PAF (2 columns)
		k. If a binary outcome, columns for a, b, c, and d, from a 2x2 table (I.E. cases in comparison level, non-cases in comparison level, cases in reference level, non-cases in reference level).
	9. If list of Y and/or A, repeat for all specified combinations of Y and A
		a. Estimate unadjusted and adjusted
	10. Clustered standard errors for recovery outcomes when there are repeated measures on 1 child (multiple cases of wasting). Better way to do this analysis?
	11. Any SL/TMLE diagnostics?
		a. Rules for adjusting when Y is sparse - only include a few covariates
		b. Positivity diagnostics 
	12. Sensitivity analyses (look at analysis plan - would these just be additional Y's?). 
	13. Pool RR's and ATE's using both random and fixed effects meta-analyses (and overall SL?)
	14. Let them know that the input dataset will be updating through the next couple of months,
	15. Synapse code to post figures/results to synapse
	16. Prioritizations for the output (generate the results first, then plots, etc.)
		a. TMLE estimation of RR and PAF
		b. Unadjusted output df
		c. Adjusted output df
		d. Subgroup analysis
		e. Forest plots
		f. Pooled analysis plots
Shiny app
...
```{r}
knitr::kable(head(wastinc_024_adj[[1]],10))
```
...
head(wastinc_024_unadj$W1)


```


### Wishlist to add to the above
  - xxx
...
#So now, for a single outcome, we have a dataframe of unadjusted means and RR for all risk factors of interest, and a dataframe
# of means (currently unadjusted, could be replaced with adjusted) + adjusted RR's for all risk factors of interest

#Below, we will generate the plots of interest for this outcome, but beyond what we've calculated here, we'll:
    # -Need to add in PAF estimation
    # Repeat for all Y
    # Repeat for all age strata (0-6 months of age, 0-24 months of age, 6-24 months of age)
    # All of the above repeated, stratified by a-priori effect modifiers
    # Also repeat some of above for sensitivity analyses

#-add code to make it easy to use a different W for each risk factor


#Modular plots:
#-make it easy to do a plot of all risk factors with a set outcome, or all outcomes and a single risk factor, or subset to a region, etc.

#Two types of plots:
   #-Forest plots of study-specific RR's from 1 Y and 1 A across all cohorts, as well as pooled RR's
   # -Pooled RR plots, faceted across risk factors or outcomes

#Maybe also print tables of the numbers underlying the forest plots

#It may be easiest to set up a markdown report that estimate and saves results from one Y and age strata, and all risk factors (like above)
#But I want a way to easily compile the results dataframes into a single dataframe with results from all Y, A, and age-strata and effect modifier subgroups.
...
# And print forest plots for all risk factors
#-----------------------------------
...
# 1) table of N's and means by study and RF level
# 2) Pooled RF plots
# 3) any TMLE3 diagnostics
# 4) Individual RF forest plots


#-----------------------------------
# List of desired changes from above
#-----------------------------------
...
#### Function to impute missingness function
